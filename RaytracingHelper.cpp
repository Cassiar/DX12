...
// --------------------------------------------------------
void RaytracingHelper::Raytrace(std::shared_ptr<Camera> camera, Microsoft::WRL::ComPtr<ID3D12Resource> currentBackBuffer, unsigned int raysPerPixel, unsigned int maxRecursion)
{
	if (!dxrAvailable || !helperInitialized)
		return;
